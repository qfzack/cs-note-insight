...
虚拟内存是一种内存管理技术，通过将物理内存和磁盘空间结合使用，为每个进程提供独立的地址空间，实现内存的扩展和保护，可以简单理解为操作系统让程序看到假的地址空间，并把不常用的数据放在磁盘来扩展内存

虚拟内存的核心是MMU（内存管理单元）和页表（Page Table）：

1. 进程访问虚拟地址（Virtual Address）
2. MMU查找页表，将虚拟地址转换为物理地址（Physical Address）
3. 如果页表中没有对应的映射，触发缺页异常（Page Fault）
4. 操作系统将所需的页面从磁盘加载到物理内存，更新页表
5. 继续访问物理地址
...
- 代码区（Text Segment）
  - 存储编译后的机器代码，是一个只读区域，避免程序的意外修改指令
  - 程序启动时加载进内存
  - 多个进程可以共享同样的代码段
- 全局/静态区（Data Segment）
  - 存放全局变量、静态变量（包括类静态成员）
  - 分为初始化（.data）和未初始化（.bss）
- 常量区（Read-only Segment）
  - 存放字符串常量和const修饰的常量，根据实现可能放在只读区或者data/bss区
- 栈区（Stack）
  - 存放函数的局部变量、函数参数和返回地址、回溯信息、临时变量
  - 特点是先进后出，由编译器自动管理，生命周期随函数调用结束而释放，并且空间有限
- 堆区（Heap）
  - 需要手动管理的区域，空间较大

## 内存对齐是什么

内存对齐是让变量在内存中的起始地址符合特定规则，以提升CPU的访问速度，为了让CPU更快和更安全地访问内存，变量会被放在若干字节对齐的地址上，而不是随便放

常见的对齐有1字节、2字节、4字节、8字节等

对齐规则:

1. 基本类型对齐: 基本数据类型（如int、float、double等）通常按照其大小进行对齐，例如4字节的int按4字节对齐
2. 结构体对齐: 结构体的对齐方式通常是其最大成员的对齐方式，同时结构体的总大小也会被调整为最大成员对齐的倍数
   - 成员对齐为4字节，整体大小为12字节：
   ...
   - 调整成员顺序减少padding
    ...
    // Offset 0-3: int b (4 bytes)
    // Offset 4-5: short c (2 bytes)
    // Offset 6: char a (1 byte)
    // Offset 7: padding (1 byte)
    ```

3. 自定义对齐: 可以使用`#pragma pack`或`alignas`来指定自定义的对齐方式
    ...
    sizeof(PackedStruct); // 输出7
    ```

## 动态链接库如何装载到内存

- 动态链接库类型
  - 共享库（Shared Library），在Linux中通常以`.so`为后缀，在Windows中以`.dll`为后缀
- 动态链接器
  - 负责在程序运行时加载动态链接库，进行符号解析、重定位、初始化DLL
  - Linux系统中通常是`ld.so/ld-linux-x86-64.so.2`，Windows系统中是`Loader`

动态链接库不会在编译时装载，而是在程序运行时由OS的动态链接器（Dynamic Linker）负责装载到内存中，通过重定位和符号解析，让程序可以调用其中的函数

动态链接库的装载过程：

1. 程序启动时，动态链接器管理动态库的加载
2. 查找动态库，loader根据环境变量（如`LD_LIBRARY_PATH`）、idconfig缓存和默认路径（`/lib/`, `/usr/lib`）的顺序查找动态库文件
3. 把动态库映射到进程地址空间（mmap）
   - .text（代码区）段映射为只读可执行
   - .data（初始化数据）段映射为可读写
   - .bss（未初始化数据）段映射为可读写
   - .rodata（常量区）段映射为只读
4. 符号解析和重定位
   - 调用动态库中的函数，loader会查找符号表，找到函数地址，并更新程序中的调用地址
   - 如果动态库依赖其他动态库，loader会递归加载这些依赖库
5. 初始化动态库
   - 如果动态库.so中有构造函数或初始化代码，loader会在装载后执行这些初始化代码
6. 程序运行时调用动态库函数

## 函数压栈是什么

函数调用的压栈是指在调用函数的时候，把必要的数据按顺序存入栈中，以便：
...
压栈就是把：返回地址、寄存器状态、参数、局部变量等信息存入栈的栈帧中，这样才能保证函数安全运行、退出后恢复现场
...
1. 保存返回地址: 调用函数前，CPU会将下一条指令的地址（返回地址）压入栈中
2. 保存寄存器状态: 保存当前函数使用的寄存器状态，以便函数返回时恢复
3. 分配栈空间: 为函数的局部变量和临时数据分配栈空间
4. 传递参数: 将函数参数压入栈中，或者通过寄存器传递
5. 跳转到函数体: 执行函数体代码
6. 函数返回时，恢复寄存器状态，释放栈空间，跳转回返回地址继续执行

## 堆和栈的区别

堆（Heap）和栈（Stack）是两种不同的内存管理方式，它们在内存分配、生命周期、访问速度等方面有显著区别：

- 内存分配方式:
  - 栈: 由编译器自动管理，函数调用时分配，函数返回时释放
  - 堆: 由程序员手动管理，通过`new`和`delete`进行分配和释放
- 生命周期:
  - 栈: 生命周期与函数调用相关，函数结束时自动释放
  - 堆: 生命周期由程序员控制，直到显式释放内存
- 访问速度:
  - 栈: 访问速度快，数据局部性好
  - 堆: 访问速度较慢，可能导致内存碎片
- 内存大小:
...
`new`运算符用于动态分配内存，也就是在运行时向堆申请空间，相比之下普通局部变量在栈上分配空间，生命周期由编译器管理，当对象的生命周期需要脱离当前作用域、大小运行时才能确定、或数量不固定时使用`new`

- 需要对象的生命周期脱离当前作用域

  ```cpp
  Foo* create() {
    return new Foo();  // 分配在堆上，函数返回后仍然存在
  }

  Foo f;
  return &f; // 分配在栈上，生命周期结束变成悬空指针
  ```
...
实际更推荐使用智能指针（如`std::unique_ptr`、`std::shared_ptr`）来管理动态分配的对象，避免内存泄漏：

```cpp
auto p = std::make_unique<Foo>();  // 使用智能指针管理动态分配的对象
auto sp = std::make_shared<Foo>();
```

对于vector可以选择放在栈或堆上（new），但其底层数组会放在堆上，因为需要动态变化

## new和malloc的区别

`new`和`malloc`都是用于动态内存分配的，但它们在功能、用法和行为上有显著区别：

- 类型安全:
  - `new`: 返回指定类型的指针，具有类型检查
  - `malloc`: 返回`void*`，需要强制转换为目标类型
- 构造函数调用:
  - `new`: 会调用对象的构造函数进行初始化
  - `malloc`: 不会调用构造函数，只分配内存
- 析构函数调用:
  - `new`: 需要使用`delete`释放内存，会调用析构函数
  - `malloc`: 需要使用`free`释放内存，不会调用析构函数
- 内存分配失败处理:
  - `new`: 分配失败时抛出`std::bad_alloc`异常
  - `malloc`: 分配失败时返回`NULL`指针
- 语法:
  - `new`: 使用`new`关键字进行分配
  - `malloc`: 使用函数调用`malloc(size)`进行分配
...
class Foo {
public:
    Foo() { cout << "Constructor called" << endl; }
    ~Foo() { cout << "Destructor called" << endl; }
};

int main() {
    // 使用new分配内存
    Foo* p1 = new Foo(); // 调用构造函数
    delete p1;           // 调用析构函数

    // 使用malloc分配内存
    Foo* p2 = (Foo*)malloc(sizeof(Foo)); // 不调用构造函数
    free(p2);                            // 不调用析构函数
...
- cpp20
  ...
  - 同步器（synchronizers）和原子智能指针

## auto类型推导的原理

auto是让编译器根据初始化表达式的类型自动推导变量类型，本质上编译器在编译器就确定了类型，并不是运行时才确定类型，auto只是简化了代码书写，提高了可读性和维护性

## 怎么避免内存泄漏

- 尽量不要手动管理内存
- 使用智能指针（如`std::unique_ptr`、`std::shared_ptr`）来自动管理动态分配的对象，避免忘记释放内存
- 保证new和delete成对出现，避免遗漏释放
- 使用RAII（资源获取即初始化）原则，将资源的获取和释放绑定在对象的生命周期内
- 避免循环引用，特别是在使用`std::shared_ptr`时，可以使用`std::weak_ptr`来打破循环引用

总之就是能不用new就不用，如果必须用堆内存，一定要有对应的释放或者使用智能指针管理

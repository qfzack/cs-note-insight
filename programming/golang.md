...
- 对象：结构体（struct）的实例
- 类：结构体（struct）
- 封装：属性的访问权限通过首字母大小写来控制，首字母大写是公共可以被包外部访问的，首字母小写是包内可见的
- 继承：不支持直接继承，但是可以通过组合（composition）和结构体嵌入（embedding）来实现继承的效果
- 多态：不支持多态，但是可以通过interface定义方法，不同实例有不同的实现，从而达到多态
...
- 编译速度快，原生支持交叉编译，可以静态编译为单个二进制文件，部署方便，执行性能接近C
...
- **值拷贝语义**：不管哪个版本，for-range 循环中得到的变量 `v` 始终是元素的**副本**，而不是引用，修改 `v` 不会影响原始切片/数组中的数据
- **变量复用语义（版本差异）**：
  - **Go 1.21 及之前**：循环变量 `v` 是被**复用**的（地址不变，只是值在变）。如果在循环中启动 goroutine 并被闭包捕获，所有 goroutine 往往都会读到最后一次迭代的值
  - **Go 1.22 及之后**：循环变量 `v` 在每次迭代时都会**重新创建**（地址会变）。闭包捕获不再有问题，可以直接在 goroutine 中使用 `v`
- **最佳实践**：为了代码的向后兼容性和安全性，在 goroutine 中使用循环变量时，习惯上依然推荐通过参数传递 `go func(val T){...}(v)` 或在循环体内创建新的变量接收 `v` 的值
...
`struct{}`的底层大小是0字节，而`interface{}`在底层由两个指针组成（一个指向类型信息，一个指向实际数据），在64位系统上通常是16字节，另外`interface{}`会导致变量逃逸到堆上（因为没有具体类型的信息，并且内部持有指针，编译器会将其分配到堆上），增加GC压力
...
- 检查类型是否实现了指定接口，`var _ io.Reader = (*MyReader)(nil)`，将`nil`强制转换为`*MyReader`类型，以此实现静态类型检查

### goroutine的参数传递有什么需要注意的

- 显示传递参数优于闭包引用外部参数
- 避免共享引用类型
- 多个goroutine共享变量时需要注意同步（使用channel做通信同步、使用sync包的锁和原子操作同步）

### 如何写go的单元测试

单元测试文件名以`_test.go`结尾，函数名以`Test`开头，接受一个`*testing.T`类型的参数
...
- 浅拷贝与深拷贝的区别在于值复制的是偶时候会复制引用的数据：
  - **浅拷贝**：只复制对象本身，不复制其引用指向的数据，多个对象共享同一个引用数据
  - **深拷贝**：不仅复制对象本身，还会递归复制其引用指向的数据，两个对象完全独立
- golang中，以下类型的变量本身是值，但是底层数据是引用或者指针：
  - slice（本身是一个结构体Slice Header，复制会复制Header，包含底层数组指针、长度、容量，底层数组共用）
  - map（复制一个指向底层hmap结构体的指针，底层哈希表共用）
  - channel（复制指向底层hchan结构体的指针）
  - interface{}（本身是一个eface结构体，复制会复制类型指针和数据指针）
  - pointer
  - function（复制一个指向指令集的指针，函数是不可变的）
- 对于chan、func不能直接深拷贝
...
- init方法是一个特殊的函数，用于在程序运行前初始化包的状态，是每个包中自动执行初始化的方法，在main之前由go运行时自动调用，没有方法参数
- 主要使用场景有：
  - 初始化变量：加载默认配置，设置常量值
  - 注册插件：如HTTP路由、数据库驱动等
...
0. 项目的监控告警实是怎么做的
1. go中如何创建协程，如何防止协程崩溃
   - 使用`go`关键字创建协程，高并发场景使用协程池，通过错误捕获和资源限制避免崩溃
2. go的数组和切片的区别
3. 介绍GMP模型
4. 为什么不是用全局队列而是每个P自己绑定一个队列
5. 介绍GC
6. 内存逃逸是什么
7. 了解grpc吗
8. 鉴别go代码中的并发加锁，并发等待，错误处理，超时控制等问题，以及是否使用了废弃的标准库
9. 服务的热更新怎么实现的，nginx的restart会导致连接断开，而reload不影响现有连接
10. 怎么设计一个长连接网关服务
11. map的panic能不能被捕获
12. 有没有看过sync.Map的源码
13. sync.Map适合读多写少的情况吗
14. channel是怎么实现的
15. 等待队列满了，有个协程要放数据，这个数据还会不会放到队列里
16. 假设有很多协程要接受数据，那么这个数据是直接给协程还是放到队列里
17. goroutine有哪些阻塞场景，怎么处理
18. 抢占式调度了解吗，system monitor协程内存分配了解吗
19. context包用过吗，实现原理了解吗
20. gin框架是做什么的，了解它的中间件原理吗
21. 讲讲sync.Mutex的实现原理
22. hand off，系统调用在一个程序中可能很常见，如果大部分都hand off不会影响性能吗
23. slice作为参数传递，外部会观测到slice的变化吗，map作为参数呢
24. 生产消费模型怎么实现，不使用channel呢
25. 怎么确定关闭channel，如何检查
26. 非法地址在go里如何判定
27. slice语法糖了解吗，OS级别的实现是什么
28. 逃逸分析机制是什么，怎么优化内存分配
29. sync.Map如何实现并发安全，和加锁map的区别是什么
30. 反射的原理是什么，哪些场景适合使用反射
31. go的协程是怎么实现的
32. 解释一下context的作用
33. 动态路由和网关的性能优化是怎么做的
34. go可以通过什么方式实现长连接
35. grpc和http的区别
36. 为什么选择socket而没有选择grpc
37. defer的执行顺序，项目中使用defer的场景有哪些
38. go怎么实现并发控制
39. 父context取消后，子context会怎么样
40. 对关闭的channel操作会怎么样
41. goroutine和线程的区别
42. 介绍下worker pool的实现原理
43. go的逃逸分析是什么
44. 协程的调度机制是什么，和进程、线程的区别
45. go中如何操作数据库
46. http协议中，go模块如何确认接收完整文件
47. http响应中最重要的模块是什么
48. CPU使用率异常的时候怎么排查
49. 日常监控的重点是什么
50. go中make和new的区别
...
53. for循环中defer怎么写
54. go中哪些类型是不可比较的
55. 项目开发中怎么排查性能瓶颈，go程序的性能瓶颈怎么排查
56. go是否属于面向对象语言，不符合面向对象的哪些特征
57. go如何实现封装继承多态
58. slice和int float有什么区别，传递slice要注意什么，如果在slice里存指针，对slice进行复制，修改新的slice会影响原slice吗
59. select多路复用原理
60. context的超时控制，未正确取消导致goroutine泄漏是什么情况
61. int和int32的区别
62. slice的底层实现是什么
63. go的channel（有无缓冲）、并发控制和关闭（创建一个控制goroutine退出的channel）、channel是否线程安全
64. select和switch的区别、执行机制
65. goroutine创建过多会引发什么问题，哪些场景会导致内存泄漏
66. GMP模型中，work-stealing机制的核心原理
67. go的协程栈是如何演进的
68. 了解go虚拟机的调度逻辑吗，以IO操作为例说明其调度流程
69. go的互斥锁是否是可重入的
70. 如何实现一个并发安全队列，会如何设计
71. 做过哪些针对高并发场景的优化，落地效果如何
72. 如何检查channel是否已关闭
73. select的底层原理，如何解决channel饥饿的问题
74. 多个case就绪，select的执行逻辑是什么，如果编码中固定case的顺序，是否会有潜在的问题
75. go的参数传递是传值还是传引用，包括slice、map、channel这些类型的底层传递逻辑是什么
76. 实现map并发，sync.map和sync.mutex的区别是什么
77. channel和select的使用场景
78. channel有缓存的情况下，怎么选择缓存大小
79. go有GC机制，还会有内存泄漏吗
80. go-zero框架了解吗
81. gin框架的优势
82. 使用了go的哪些标准包
83. sync中能否使用channel代替waitgroup
84. 高QPS下，不引入外部资源，如何优化sync.map的性能
85. go错误处理的理解，错误处理到日志、可观测性的最佳实践
86. 线上服务问题（如内存泄漏）的排查方式
87. context包的核心作用，解决了什么问题，有哪些必要的使用场景
88. 竟态检测-race怎么使用
89. channel的底层结构是什么
...
- 作用：
  - 管理容器生命周期：保证Pod信按照定义被创建、运行、重启、终止
  - 读取podSpec：从API server或本地static pod文件中获取pod描述
  - 与container runtime通信：通过CRI控制containerd/CRI-O创建、删除容器
  - volumn管理：挂载PVC、临时目录、configMap、Secret等volumn资源
  - 健康检查：负责执行Liveness、Readiness、Startup探针，报告容器是否健康
  - 上报节点/pod的状态：定期将Node状态（资源、网络等）和Pod状态报告给API server
  - 拉取镜像：下载并缓存所需镜像容器
  - 日志和事件上报：上报运行中容器的日志和事件，供用户通过kubectl查看
  - 安全隔离：创建容器前设置安全上下文
  - CNI插件对接：与CNI网络插件交互，为pod配置网络
  - CSI插件对接：与CSI存储插件交互，为pod挂载持久卷
...
RBAC是什么，有什么特点和优势
- RBAC是基于角色的访问控制，是K8s中用于控制用户或服务账户访问权限资源的机制
- RBAC的核心资源
  - Role：定义在命名空间级别的权限
  - ClusterRole：定义在集群级别的权限
  - RoleBinding：将Role绑定给用户/组/服务账户（命名空间级）
  - ClusterRoleBinding：将ClusterRole绑定给用户/组/服务账户（集群级）
- RBAC工作机制
  - 用户发起请求，如创建Pod
...
secret的作用
- Secret用于存储敏感信息的资源对象，例如密码、token、docker镜像拉取凭据和SSH密钥等，可以被pod挂载或作为环境变量使用
- 作用：
...
- 作为镜像拉取凭证：配置在`imagePullSecrets`使在镜像拉取时能够使用正确的认证信息
> 当pod使用service account时，k8s会自动将对应的secret（类型为service-account-token）注入到容器，用于pod内部访问k8s API

PodSecurityPolicy机制是什么，能实现哪些安全策略
- PSP是k8s的一种准入控制器，用于限制和控制pod的安全相关配置，是一种集群资源，属于强制策略，不符合规则的pod无法创建，在1.25被正式移除，建议使用PodSecurity admission、OPA、Kyverno代替
- PSP的机制：
...
K8s的网络策略及其原理
- 网络策略（NetworkPolicy资源类型）是一种基于标签的规则集，用于定义哪些pod可以相互通信，哪些网络流量被允许或拒绝，允许用户通过申明式的方式，限制pod之间及pod与外部的网络访问，实现细粒度的网络隔离
- 网络策略的核心概念：
  - Pod Selector：通过标签选择目标Pod，指定规则作用对象
  - Ingress规则：允许进入目标Pod的流量
  - Egress规则：允许目标Pod发出的流量
  - Policy Types：指定应用规则的方向（Ingress、Egress）
- 工作原理
...
K8s中flannel的作用
- Flannel是一种K8s网络插件（CNI插件），用于为Pod提供跨主机通信的虚拟网络
- 作用：
...
Calico网络组件的实现原理
- Calico是一个功能强大的k8s网络方案，既是CNI插件，又提供了丰富的网络安全策略支持
- 主要特点：
...
PV和PVC是什么
- Persistent Volume (PV)：集群管理员或者storage class创建的持久化存储卷，用于提供存储资源，并描述了容量、访问模式、存储类型，是集群级别的资源
...
K8s支持的存储供应模式有哪些
- 静态供应（Static Provisioning）：集群管理员手动创建PV，用户创建PVC后绑定合适的PV，适用于对接传统存储，但是维护成本高、效率低
- 动态供应（Dynamic Provisioning）：用户创建PVC，K8s根据PVC中指定的StorageClass自动创建PV，完成PV-PVC的绑定，自动化程度高、支持弹性扩容

K8s的CSI模型
- CSI模型是什么
  - CSI（Container Storage Interface）是一套由CNCF推出的容器存储接口标准，用于统一容器编排系统与各类存储系统之间的对接方式
- 为什么需要CSI
...
Worker节点加入集群的过程
- worker节点是实际运行pod的节点，必须与集群的control plane通信，才能被调度金额管理，加入集群的过程是通过kubeadm向master节点注册，并启动必要组件（如kubelet、kube-proxy）
- node注册到集群
...
Metric Service是什么
- Metrics Server是 Kubernetes中用于收集、聚合和暴露集群资源使用情况的组件，供k8s系统或者外部系统（如Prometheus、HPA）进行调度决策、自动扩缩容和可观测性分析
- 主要作用：
...
- 服务注册机制：
  ...
  - DNS注册阶段：CoreDNS监听API server的service变化，为新service自动创建DNS记录，如`mysvc.default.svc.cluster.local
`，应用可以通过DNS名称解析service IP进行访问
  - Endpoints Controller自动将匹配的Pod列表写入到Endpoints对象中
> k8s的服务注册是自动完成的，Pod和Service都是由控制面组件负责注册和维护

K8s和Docker swarm的区别

什么是Container Orchestration，需要什么
...
replica set和replication controller之间有什么区别

什么是headless service

考虑一家拥有分布式系统的跨国公司，拥有大量数据中心，虚拟机和许多从事各种任务的员工，这样的公司如何以与K8s一致的方式管理所有任务

如何设计一个更具可扩展性和响应性的平台

从单机到微服务的转变解决了开发方面的问题，但是却增加了部署的问题，这如何解决

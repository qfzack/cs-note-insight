...
- 触发条件：
  - 定时触发：配置文件中设置save参数，如`save 900 1`表示900秒内至少有1个key被修改就触发
  - 手动触发：执行BGSAVE命令
  - 主从复制：主服务器在有从服务器连接时会自动触发BGSAVE
...
- 大量缓存数据同一时间过期/失效或者Redis服务宕机，此时如果有大量请求，都无法在Redis处理而全部请求到数据库，导致数据库压力过大
- 解决方案：
  - 设置均匀过期时间：设置均匀的缓存过期时间，避免同一时间大量缓存过期
  - 互斥锁重建缓存：处理请求时，如果发现缓存失效，可以设置一个互斥锁，只有第一个请求可以访问数据库重建缓存，其他请求等待锁释放后再访问缓存
  - 设置热点数据不过期：对于一些热点数据，可以设置为永不过期，避免缓存雪崩

**缓存击穿**

- 缓存中的某个热点数据过期，此时如果有大量请求访问该热点数据就无法从缓存读取，而直接访问数据库，导致数据库压力过大
- 解决方案：
  - 互斥锁重建缓存
  - 设置热点数据不过期
...
- 用户请求的数据既不在缓存中，也不在数据库中，导致请求在访问缓存的时候发现缓存缺失，然后访问数据也无法重建缓存，大量的请求也会导致数据库压力过大
- 解决方案：
  - 限制非法请求：当有大量恶意请求访问不存在的数据时，也会发生缓存穿透，因此可以在API请求的时候判断请求参数是否合法、请求字段是否存在，非法请求直接返回错误，避免进一步访问缓存和数据库
  - 缓存空值或者默认值：当发现存在缓存穿透现象时，可以针对查询数据在缓存中存储一个空值或者默认值，避免后续请求继续访问数据库
  - 使用布隆过滤器：在写入数据库的时候，使用布隆过滤器做个标记，然后在用户请求到来的时候，业务线程确认缓存失效后，先通过查询布隆过滤器快速判断数据是否存在，如果不存在就不用访问数据库，这样即使发生缓存穿透，大量请求也只会查询缓存和布隆过滤器，避免数据库压力过大

### 布隆过滤器

布隆过滤器（Bloom Filter）是一个空间效率很高的概率型数据结构，可以用来判断一个元素是否在一个集合中，其特点是：

- 可能误判存在
- 不会误判不存在
- 内存占用很小，适合百万计甚至亿级别的数据判断

对比hashset，布隆过滤器的特点在于：

- 内存占用更小：hashset需要存储每个元素的原始数据，而布隆过滤器只需要存储位数组，节省大量内存
- 结构简单：布隆过滤器只需要一个位数组和多个哈希函数，而hashset需要复杂的数据结构来存储元素
- 侧重判断元素不存在：布隆过滤器主要用于快速判断元素是否不存在，而hashset主要用于存储和查找元素

布隆过滤器内部是一个固定大小的位数组（初始值为0），当添加一个元素时：

1. 使用多个哈希函数对该元素进行哈希运算，得到多个哈希值
2. 将这些哈希值对应的位数组位置设置为1

查询元素时：

1. 使用相同的哈希函数对该元素进行哈希运算，得到多个哈希值
2. 检查这些哈希值对应的位数组位置
3. 如果都是1，则认为该元素可能存在；如果有任何一个位置是0，则认为该元素一定不存在

因为不同元素可能会映射到相同的某个位置，所以布隆过滤器可能会误判一个元素存在，但不会误判不存在

Redis也支持布隆过滤器，可以通过Redis模块RedisBloom来使用布隆过滤器功能

### Redis管道

Redis的管道是一种批量发送命令的机制，用来减少客户端与Redis服务器之间的网络往返时间（RTT）

Redis单条命令的执行很快，但是网络来回延迟会远高于命令执行时间，因此如果每次发送一条命令都要等待网络往返时间，会大大降低整体吞吐量

Pipeline可以把多条命令一次性批量发送到Redis，然后一次性读取所有命令的返回结果：

```bash
{ echo "SET key1 111"; echo "SET key2 222"; echo "INCR key1"; } | redis-cli --pipe
```

### Redis的事务回滚

redis事务不支持回滚，即不保证事务的原子性，一旦执行了`EXEC`，即使事务中的某条命令执行失败，其他命令也会被执行，并且无法回滚

通常会结合`WATCH`命令来实现乐观锁机制，确保在执行事务前数据没有被其他客户端修改，当使用`WATCH`监视的key被其他客户端修改时，事务会被中止返回nil，客户端可以选择重试事务
...
秒杀场景下并发量非常高，并且时间短，要求数据库与缓存的强一致性，防止超卖

如果是直接使用数据库，会存在DB扛不住QPS、行锁竞争严重、TPS（每秒事务数）低、扩展性差等问题，因此秒杀场景通常会在Redis层完成

**基础方案**：使用Lua脚本+Redis INCR/DECR

- Redis的INCR/DECR命令是原子操作，可以用来实现库存的扣减，但是存在超卖风险
- Lua脚本是原子执行的，可以在脚本中先检查库存是否充足，再进行扣减，避免超卖，然后创建订单写入数据库
...
- 进阶防止一个用户多次秒杀，可以在Lua脚本中加入用户ID的检查，确保每个用户只能秒杀一次
- 如果Redis库存扣减成功，但是写入数据库失败，可以通过异步补偿机制（如消息队列）来处理，确保最终一致性

**改进方案**：Redis+消息队列削峰

- 用户的秒杀请求不等于下单请求，可以先将秒杀请求放入消息队列中，异步处理下单逻辑
- 整体流程为：
  1. 系统初始化的时候，将商品库存预先加载到Redis中
  2. 接收到用户的秒杀请求
  3. 使用Redis Lua脚本：原子性操作检查库存，如果库存充足则扣减库存
  4. 将秒杀请求放入消息队列，返回正在排队提示给用户
  5. 后台消费者从消息队列中异步消费请求，创建订单并写入数据库
  6. 写入数据库的时候需要再次检查库存，防止超卖
  7. 返回秒杀结果给用户

热key的问题：如果所有的请求都集中在同一个key上，而Redis单key的并发处理能力有限，可能会导致该key所在节点成为性能瓶颈，解决方案有：

1. 在Redis集群架构中对热key复制并迁移到其他节点的分片，来解决单个数据分片的热key压力过大问题
   - 例如原来的key是`product:1001:detail`，把这同一个数据写到多个key，落在不同的节点，写数据时也同步更新所有副本（读多写少），访问的时候在Lua脚本随机请求一个key
   ...
   product:1001:detail:3   → Node D
   ```
2. 本地缓存+Redis：在应用服务器本地使用本地缓存（如Caffeine、Guava Cache等），减少对Redis的访问压力

Redis宕机怎么办？

- 对服务进行熔断降级，直接返回秒杀失败，直接返回活动火爆的提示
